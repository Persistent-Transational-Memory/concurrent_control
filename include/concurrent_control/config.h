...
#pragma once
#ifndef CONCURRENT_CONTROL_CONFIG_H
#define CONCURRENT_CONTROL_CONFIG_H
...
namespace cc {
	...
	constexpr bool GlobalRecordSwitch[] = {
			[(uint32_t)RecordEvent::running]      = false,
			[(uint32_t)RecordEvent::commit]       = false,
			[(uint32_t)RecordEvent::index]        = false,
			[(uint32_t)RecordEvent::validate]     = false,
			[(uint32_t)RecordEvent::persist_log]  = false,
			[(uint32_t)RecordEvent::persist_data] = false,
...
//			util::get_global_logger().print_property("Thread Latency",
...
#endif //CONCURRENT_CONTROL_CONFIG_H
