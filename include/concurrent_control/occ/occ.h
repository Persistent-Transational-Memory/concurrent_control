...
#pragma once
#ifndef CC_OCC_ATOMIC_H
#define CC_OCC_ATOMIC_H
...
namespace cc::occ {
	...
	public: // Class Property
		...
		explicit OCC(StorageManager *storage_manager_ptr): time_counter(0),
			...
			util::logger_warn("Atomic version of OCC can't make sure that data read is integral when running transaction, "
			...
			if (has_write) {
				...
				if (success_validate) {
					for (auto &entry: read_set) {
						IndexTupleType &origin_tuple = entry.tuple;
						if (origin_tuple.is_locked_write()) {
	...
	private: // Assist Function
		...
		void write_log(Context &tx_context) {
			...
			auto log_space = log_persist_.allocate_log_space(log_amount, log_size);

			// ---- Write log
			// Start
			log_persist_.add_start_log(log_space, commit_ts);
			// Insert
			for (const InsertEntryType &entry: insert_set) {
				log_persist_.add_insert_log(log_space, commit_ts, entry.key, entry.data_ptr, entry.size);
			}
			// Write
			for (const WriteEntryType &entry: write_set) {
				if (entry.type == TxType::Write) {
					log_persist_.add_update_log(log_space, commit_ts, entry.key, entry.data_ptr, entry.size, entry.offset);
				}
				else {
					log_persist_.add_delete_log(log_space, commit_ts, entry.key);
				}
			}
			// Commit
			log_persist_.add_commit_log(log_space, commit_ts);
